<?php
    ...
    if(isset($_POST['btnSubmit'])){
        $sql = "UPDATE tblequipment SET model = ?, description = ?, department = ?, status = ? WHERE serial_number = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssss", $_POST['model'], $_POST['description'], $_POST['department'], $_POST['status'],  $_GET['serial_number']);
            
            if(mysqli_stmt_execute($stmt)){
                header("Location: Equipment-Management-Menu.php?success=1");
                $_SESSION['message'] = 'Equipment successfully updated!';
                exit();
            } else{
...
<head>
    ...
    <link rel="stylesheet" href="Style_Equipment-Update.css">
</head>
<body>

    <div class ="formContainer">

        <p class="updateEquipmentInstruction">Edit the values and submit to update the equipment</p>

        <form action = "<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method ="POST">

        <div class ="dataSerialNumber">
            <label>Serial Number: </label><?php echo $row['serial_number']; ?>
        </div>
        ...
        <div class ="dataBox">
            ...
            <select name="status" id="model" required>
